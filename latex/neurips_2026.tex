...
The abstracts of clinical trials contain important information about study design, participant groups, treatments, and results. However, this information is presented in unstructured text, making it difficult to extract and compare efficiently across studies. Converting such abstracts into structured tables can support faster evidence and make relevant information easier to locate and analyse. This creates a useful setting for studying how different information extraction strategies perform on biomedical text.


\subsection{Task Overview}


This project addresses the task of structured information extraction from clinical trial abstracts in the \textbf{EBM-NLP} dataset. The objective is to identify and extract \textbf{Population, Intervention, Comparator, and Outcome (PICO)} information from unstructured abstracts and represent it in a predefined schema.

The task also includes preliminary clustering analysis and comparison of alternative extraction pipelines. We then evaluate these approaches in terms of extraction quality and discuss the trade-offs and limitations of the task.
...
A useful extraction system must therefore not only achieve good field-level performance, but also balance coverage and precision while producing outputs that remain usable for downstream querying and analysis.

\subsection{Dataset and Problem Characteristics}

The EBM-NLP dataset contains clinical trial abstracts paired with annotated target fields for structured information extraction. In this task, the goal is to recover key study information from unstructured abstracts and map it into a predefined schema containing Population, Intervention, Comparator, and Outcome elements. The coursework brief also emphasises that the available data should be described clearly, with examples from the dataset where appropriate. 

Table~\ref{tab:ebm_example} shows an example abstract together with corresponding annotated fields. This example illustrates that important information is embedded within longer biomedical sentences rather than presented in an explicitly structured format. It also shows that some fields are expressed more directly than others, while certain details may appear in compact or overlapping phrasing, making extraction non-trivial.
...
This example highlights a key difficulty of the task: important trial information is embedded within continuous biomedical text rather than presented in an explicitly structured form. Some fields are expressed relatively clearly, while others may appear in compact, overlapping, or less consistently annotated forms. In our setting, comparator information is also less clearly represented than the other target fields, which makes full PICO evaluation more difficult. These characteristics make the dataset suitable for comparing alternative extraction strategies, since a useful system must cope with variation in wording, structure, and field clarity across abstracts.


\subsection{Requirements for a Good Solution}

A good solution for this task must recover the correct target fields from unstructured clinical trial abstracts and map them into a usable structured format. This requires more than strong field-level extraction performance alone. The system should also balance coverage and precision, handle variation in wording and sentence structure across abstracts, and produce outputs that remain useful for downstream querying and analysis. For this reason, our study compares multiple extraction strategies rather than relying on a single pipeline, allowing us to examine how different design choices affect extraction quality and practical usability. The details of these strategies are presented in the following section.
